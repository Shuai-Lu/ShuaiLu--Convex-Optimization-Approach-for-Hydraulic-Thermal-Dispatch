function model_buildings(varargin)
...
%% Define
model.var.buildings.Tau_in = sdpvar(num_heatperiod, num_load);
model.var.buildings.h_load = sdpvar(num_heatperiod, num_load);
model.var.buildings.Tau_out = sdpvar(num_heatperiod, 1);

%% Modeling
% % Temperature equations
model.cons = model.cons + ( ...
    (model.var.buildings.h_load >= 0) : '');
model.cons = model.cons + ( ...
    (model.var.buildings.Tau_in(1,:) == ...
    ratio * data.initialParam.buildings.temperature(loc_Tau_opt) + ...
    (1-ratio) .* ...
    (model.var.buildings.Tau_out(1,1) + ...
    data.buildings.param(:,loc_R)'./data.buildings.param(:,loc_num)' .* ...
    model.var.buildings.h_load(1,:))) : ...
    'Buildings: Tau equation');
model.cons = model.cons + ( ...
    (model.var.buildings.Tau_in(2:end,:) == ...
    ones(num_heatperiod-1,1)*ratio .* ...
    model.var.buildings.Tau_in(1:end-1,:) + ...
    ones(num_heatperiod-1,1) *(1-ratio) .* ...
    (model.var.buildings.Tau_out(2:end,1)*ones(1,num_load) + ...
    ones(num_heatperiod-1,1) * ...
    (data.buildings.param(:,loc_R)./data.buildings.param(:,loc_num))' .* ...
    model.var.buildings.h_load(2:end,:))) : ...
    'Buildings: Tau equation');

% % Temperature limit
model.cons = model.cons + ( ...
    (model.var.buildings.Tau_in >= data.initialParam.buildings.temperature(loc_Tau_low)) : ...
    'Buildings: temperature limit');
model.cons = model.cons + ( ...
    (model.var.buildings.Tau_in <= data.initialParam.buildings.temperature(loc_Tau_up)) : ...
    'Buildings: temperature limit');
model.cons = model.cons + ( ...
    (model.var.buildings.Tau_in(end,:) == ...
    data.initialParam.buildings.temperature(loc_Tau_opt)) : ...
    'Buildings: endpoint temperature');

% % Average Temperature
model.cons = model.cons + ( ...
    (ones(num_load, num_heatperiod)*model.var.buildings.Tau_in == ...
    num_heatperiod*data.initialParam.buildings.temperature(loc_Tau_opt)) : ...
    'Buildings: average Tau');

% % Tau_out
model.cons = model.cons + ( ...
    (model.var.buildings.Tau_out == ...
