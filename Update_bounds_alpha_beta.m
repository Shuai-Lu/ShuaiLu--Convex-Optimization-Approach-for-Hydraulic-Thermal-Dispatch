function Update_bounds_alpha_beta()
global data model;
...
num_iter = length(model.record.bounds);
%%
...
t = 1:num_heatperiod;
for j = 1:num_pipe
    Num_aux(t,j) = ceil(rho_w*Area_pipe(j)*Length_pipe(j)./(model.record.bounds(end).heatingnetwork(1).Massflow_min(t,j)*data.interval.heat)) + 1;
end
...
model.record.bounds(end).heatingnetwork.aux_alpha_max = aux_alpha_max;
model.record.bounds(end).heatingnetwork.aux_alpha_min = aux_alpha_min;
model.record.bounds(end).heatingnetwork.aux_beta_max = aux_beta_max;
model.record.bounds(end).heatingnetwork.aux_beta_min = aux_beta_min;

% if num_iter >= 2
%     min(model.record.solution(end).solution.heatingnetwork.aux_alpha(:)-aux_alpha_min(:))
%     min(-model.record.solution(end).solution.heatingnetwork.aux_alpha(:)+aux_alpha_max(:))
%     min(model.record.solution(end).solution.heatingnetwork.aux_beta(:)-aux_beta_min(:))
%     min(-model.record.solution(end).solution.heatingnetwork.aux_beta(:)+aux_beta_max(:))
%     min(min(model.record.solution(end).solution.heatingnetwork.Massflow(11:end,:)- ...
%         model.record.bounds(end).heatingnetwork.Massflow_min(:,:)))
%     min(min(-model.record.solution(end).solution.heatingnetwork.Massflow(11:end,:)+ ...
%         model.record.bounds(end).heatingnetwork.Massflow_max(:,:)))
% end
end