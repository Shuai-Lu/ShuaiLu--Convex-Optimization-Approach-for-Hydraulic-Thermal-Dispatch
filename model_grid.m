...
%% ---------------------- Define Variables -------------------------
    function define_vars()
        % % pcc
        model.var.grid.pcc.p_state = binvar(num_period, 2, 'full');
        model.var.grid.pcc.q_state = binvar(num_period, 2, 'full');
        ...
        % % % es
        if ~isempty(set_es)
            model.var.grid.es.p_chr_state = binvar(num_period, length(set_es), 'full');
            model.var.grid.es.p_dis_state = binvar(num_period, length(set_es), 'full');
        ...
        % %  gt
        if ~isempty(set_gt)
            model.var.grid.gt.state = binvar(num_period, length(set_gt), 'full');
        ...
        % % tst
        if ~isempty(set_tst)
            model.var.grid.tst.h_chr_state = binvar(num_period, length(set_tst), 'full');
            model.var.grid.tst.h_dis_state = binvar(num_period, length(set_tst), 'full');
        ...
        % % pump
        model.var.grid.pump.p = sdpvar(num_period, 1, 'full');
